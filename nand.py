...
bot = telebot.TeleBot('6436450622:AAEEBhg4dRpQlpbPNHtrth-545ch0tq0MFs')
...
def send_periodic_message():
    while True:
        for user_id in allowed_user_ids:
            try:
                bot.send_message(user_id, '''ᴀʟɪᴠᴇ⏤͟͟͞͞★🕊️''')
            except Exception as e:
...
def start_attack(user_id, target, port, time):
    ...
    try:
        ...
        response = f"ᴇɴᴢᴏʏ ʙʀᴏᴛʜᴇʀ ❤️. Target: {target} Port: {port} Time: {time}"
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name}, ᴀɢᴀʀ ʙᴀɴᴋ ᴍᴇ ᴘᴀɪsᴀ ᴀᴜʀ ɪɪɢᴀʀ ᴍᴇ ᴅᴀᴍ ʜᴀɪ ʙʜᴏsᴅɪᴡᴀʟᴇ ᴛᴏ ᴏᴡɴᴇʀ ᴋᴏ ᴅᴍ ᴋᴀʀᴋᴇ ᴘᴜᴄʜʜ ʟᴇɴᴀ ᴘʟᴀɴ. ʏʜᴀ ᴍᴀᴛ ᴀᴘɴɪ ᴍᴜᴍᴍʏ ᴄʜᴜᴅᴡᴀɴᴇ ᴋᴇ ʟɪʏᴇ sᴘᴀᴍ ᴋᴀʀ
'''
    bot.reply_to(message, response)
